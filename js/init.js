...
if (!carr) carr = "INF";
let sct = false;
if (params.get("SCT") === "true") sct = true;
...
includes.forEach((include) => {
  let fileURL =
    relaPath +
    "views/" +
    include.attributes["data-include"].nodeValue +
    ".html";
  promises.push(
    fetch(fileURL)
...
$(function () {
  ...
  if (prioridad) {
    ...
    document
      .getElementById("custom-credits-UDP")
      .addEventListener("input", function updateSCTPlaceholder() {
        if (this.value == "")
          document
          ...
          document
            ...
            .setAttribute(
              "placeholder",
              Math.round((this.value * 5) / 3).toString()
            );
      });
    ...
    document
      .getElementById("custom-creditsa-UDP")
      .addEventListener("input", function updateSCTPlaceholder() {
        if (this.value == "")
          document
    ...
    document
      ...
      .addEventListener("click", () => malla.cleanSubjects());
