class Priorix extends SemesterManager {
  ...
  addSubject(subject) {
    ...
    this.totalCredits["UDP"] =
      this.totalCredits["UDP"] + subject.getUDPCredits();
      ...
      if (semester !== this.semester) {
        ...
        if (found !== -1) {
          ...
          if (semester < this.semester) {
            ...
            let scoreToDeleteUDP = grade * subject.getUDPCredits();
            ...
            for (semester; semester < this.semester; semester++) {
              if (grade > 54) {
                this.totalApprovedCredits["UDP"][semester] -=
                  subject.getUDPCredits();
                this.totalApprovedCredits["SCT"][semester] -=
                  subject.getSCTCredits();
              }
              this.prevSemesterSums["UDP"][semester] -= scoreToDeleteUDP;
  ...
  unDisplaySubject(subject) {
    this.displayedSubjects[subject.sigla][1].on("input", null);
  ...
  nextSemester() {
    ...
    this.prevSemesterSums["UDP"][this.semester] =
      this.currentSemesterSum["UDP"];
    this.prevSemesterSums["SCT"][this.semester] =
      this.currentSemesterSum["SCT"];
    ...
    if (this.selectedPerSemester[this.semester]) {
      ...
      backup.forEach((subject) => {
        subject.selectRamo();
        this.displayedSubjects[subject.sigla][1].property(
          "value",
          this.subjectGrades[this.semester][subject.sigla]
        );
        subjectsToUpdate.push(subject);
      });
      this.selectedPerSemester[this.semester] = backup;
  ...
  prevSemester() {
    ...
    else backup = [...this.selectedPerSemester[this.semester]];
    ...
    if (backup !== []) this.selectedPerSemester[this.semester] = backup;
    ...
    for (i; i < Object.keys(this.subjectGrades).length; i++) {
      ...
      Object.keys(this.subjectGrades[i + 1]).forEach((sigla) => {
        if (this.malla.ALLSUBJECTS[sigla] !== undefined)
          ...
          subjectsNotFound[i + 1].push([
            sigla + ": " + this.subjectGrades[i + 1][sigla],
          ]);
  ...
  calculate() {
    if (this.calculationsEnabled) {
      let currentApprovedCreditsUDP, currentApprovedCreditsSCT;
      let currentSemesterSumUDP, currentSemesterSumSCT;
      let semesterGrades = {};
      if (this.semester !== 1) {
        currentApprovedCreditsUDP =
          this.totalApprovedCredits["UDP"][this.semester - 1];
        currentApprovedCreditsSCT =
          this.totalApprovedCredits["SCT"][this.semester - 1];

        currentSemesterSumUDP = this.prevSemesterSums["UDP"][this.semester - 1];
        currentSemesterSumSCT = this.prevSemesterSums["SCT"][this.semester - 1];
      } else {
        currentApprovedCreditsUDP = 0;
        currentApprovedCreditsSCT = 0;
        currentSemesterSumUDP = 0;
      ...
      this.selectedPerSemester[this.semester].forEach((subject) => {
        ...
        if (grade > 54) {
          currentApprovedCreditsUDP += subject.getUDPCredits();
          currentApprovedCreditsSCT += subject.getSCTCredits();
        }
        currentSemesterSumUDP =
          currentSemesterSumUDP + grade * subject.getUDPCredits();
        currentSemesterSumSCT =
          currentSemesterSumSCT + grade * subject.getSCTCredits();
      });
      let fae = this.card.select(".fae").property("value");
      let resultUDP = 0;
      let resultSCT = 0;
      if (this.totalCredits["UDP"] !== 0) {
        resultUDP =
          ((100 *
            (currentSemesterSumUDP / (14 * Math.pow(this.semester, 1.06))) *
            currentApprovedCreditsUDP) /
            this.totalCredits["UDP"]) *
          fae;
        resultUDP = Math.round(resultUDP * 100) / 100.0;
        resultSCT =
          ((100 *
            (currentSemesterSumSCT /
              (((14 * 5) / 3) * Math.pow(this.semester, 1.06))) *
            currentApprovedCreditsSCT) /
            this.totalCredits["SCT"]) *
          fae;
        resultSCT = Math.round(resultSCT * 100) / 100.0;
      }
      // this.card.select('.resUDP').text(resultUDP)
      this.card.select(".resSCT").text(resultSCT);
      // save results
      this.subjectGrades[this.semester] = semesterGrades;
      this.currentSemesterSum["UDP"] = currentSemesterSumUDP;
      this.currentSemesterSum["SCT"] = currentSemesterSumSCT;
      this.faes[this.semester] = fae;
      this.totalApprovedCredits["UDP"][this.semester] =
        currentApprovedCreditsUDP;
      this.totalApprovedCredits["SCT"][this.semester] =
        currentApprovedCreditsSCT;
      //console.log(this.totalApprovedCredits, this.totalCredits, this.prevSemesterSums, this.currentSemesterSum, this.subjectGrades)
      this.saveSemesters();
    }
