...
class Malla {
  ...
  setCareer(carr, fullCareerName, relaPath) {
    ...
    promises.push(
      d3.json(relaPath + "data/data_" + this.currentMalla + ".json")
    );
    promises.push(
      d3.json(relaPath + "data/colors_" + this.currentMalla + ".json")
    );
  ...
  drawMalla(canvasId) {
    ...
    let height =
      (this.subjectType.getDisplayHeight(this.scaleY) + separator) *
        this.longestSemester +
      semesterIndicatorHeight * 2 +
      separator;
    ...
    canvas.append("title").text("Malla " + this.fullCareerName);
    ...
    Object.keys(this.malla).forEach((semester) => {
      ...
      if (semestersPassed === 0) {
        ...
        currentYearIndicator = yearIndicator
          .append("rect")
          .attr("x", globalX)
          .attr("y", globalY)
        ...
        semestersPassed++;
        // texto de la barra
        currentYearIndicatorText = yearIndicator
          .append("text")
          .attr(
            "x",
            globalX + this.subjectType.getDisplayWidth(this.scaleX) / 2.0
          )
        ...
        currentYearIndicator.attr(
          "width",
          this.subjectType.getDisplayWidth(this.scaleX) * 2 + separator
        );
      ...
      semesterIndicator.on("click", () => {
        ...
        if (semester[0] === "s") semNumber = "s" + semNumber;
      ...
      Object.keys(this.malla[semester]).forEach((subject) => {
        this.malla[semester][subject].draw(
          drawer,
          globalX,
          globalY,
          this.scaleX,
  ...
  displayCreditSystem() {
    if (!this.showCreditSystem) return;
    d3.select("#credits-system").text(this.sct ? "SCT" : "UDP");
  }
  ...
  generateCode() {
    ...
    Object.keys(this.malla).forEach((semester) => {
      let key;
      if (semester.includes("s")) key = semester;
      else key = "s" + semester;
      data[key] = [];
      ...
      Object.keys(this.malla[semester]).forEach((sigla) => {
        ...
        subjectData.push(subject.getUDPCredits());
        if (subject.UDPtoSCT) subjectData.push(0);
        else subjectData.push(subject.getSCTCredits());
    ...
    if (document.getElementById("mallaCode")) {
      ...
      downloadLink1.setAttribute(
        "download",
        "data_" + this.currentMalla + ".json"
      );
      downloadLink2.setAttribute("href", URL.createObjectURL(file2));
      downloadLink2.setAttribute(
        "download",
        "colors_" + this.currentMalla + ".json"
      );
    } else {
      ...
      document
        ...
        .addEventListener("input", function (input) {
          ...
          document
            ...
            .setAttribute(
              "download",
              "data_" + input.target.value.toUpperCase() + ".json"
            );
          document
            .getElementById("dColor")
            .setAttribute(
              "download",
              "colors_" + input.target.value.toUpperCase() + ".json"
            );
