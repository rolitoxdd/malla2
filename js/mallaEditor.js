class MallaEditor {
  constructor(
    ...
    if (categoryLocation) {
      ...
      showModalButton.addEventListener(
        "click",
        this.setUpCategoryModal.bind(this, false, null)
      );
    ...
    let creatorModalId = this.customManager
    ...
    this.createSubjectModal.on("hidden.bs.modal", (e) => {
      ...
      e.target.querySelector("#custom-credits-UDP").value = "";
    ...
    modal.querySelector("#createSubject").addEventListener("click", (e) => {
    ...
    let advanced = this.customManager.getElementsByClassName(
    ...
    if (advanced.length !== 0) {
      ...
      this.advanced = true;
      let advancedCreatorModalId = advanced[0].getAttribute("data-target");
      this.createAdvancedSubjectModal = $(advancedCreatorModalId);
      advanced[0].addEventListener("click", () => {
        this.setUpModal();
      });
      this.createAdvancedSubjectModal.on("hidden.bs.modal", (e) => {
        ...
        e.target.querySelector("#custom-creditsa-UDP").value = "";
      ...
      let prerChooser = this.createAdvancedSubjectModal
        .get(0)
  ...
  setUpModal(isEdit = false, subject = null) {
    ...
    if (this.categories["Custom"])
      sectorChooser.firstElementChild.textContent =
        this.categories["Custom"][1];
    else sectorChooser.firstElementChild.textContent = this.defaultSector[1];
    let prerChooser = modal.querySelector("#prerChooser");
    Object.keys(this.semesterManager.malla.ALLSUBJECTS).forEach((sigla) => {
      let option = document.createElement("option");
      option.value = sigla;
      option.textContent =
        this.semesterManager.malla.ALLSUBJECTS[sigla].name + " | " + sigla;
    ...
    if (isEdit) {
      ...
      modal.querySelector("#custom-creditsa-UDP").value =
        subject.getUDPCredits();
      if (
        Math.round((subject.getUDPCredits() * 5) / 3) !==
        subject.getSCTCredits()
      )
        modal.querySelector("#custom-creditsa-SCT").value =
          subject.getSCTCredits();
      ...
      else this.editSubEvent = this.editSubject.bind(this, subject);
      this.createAdvancedSubjectModal
        .get(0)
  ...
  displaySubject(subject) {
    ...
    subjectCreditsCol.textContent =
      subject.getUDPCredits() + " UDP | " + subject.getSCTCredits() + " SCT";
    ...
    if (this.advanced) {
      ...
      subject.prer.forEach((prer) => {
        if (i === 0) {
          ...
          i = 1;
        } else subjectPrer.textContent += " | " + prer;
      });
    ...
    if (subject.selected) selectButton.textContent = "Deseleccionar";
    ...
    if (this.advanced) {
      ...
      editButton.addEventListener(
        "click",
        this.setUpModal.bind(this, true, subject)
      );
    ...
    if (this.advanced) subjectInfo.append(subjectPrer);
    subjectInfo.append(actionsCol);
    subjectInfo.childNodes.forEach((x) => x.classList.add("align-middle"));
    if (subjectInfo.animate) subjectInfo.animate(fadeIn, 500);
  ...
  updateState(subject) {
    ...
    if (subject.selected) {
      ...
      let i = true;
      Object.keys(this.semesterManager.selectedPerSemester).forEach(
        (semester) => {
          if (semester !== this.semesterManager.semester) {
            let found =
              this.semesterManager.selectedPerSemester[semester].indexOf(
                subject
              );
            if (found !== -1) {
              if (semester < this.semesterManager.semester)
    ...
    let subjectSelButton = this.tableList[subject.sigla][0].querySelector(
      "#sel-" + subject.sigla
    );
    if (subject.selected) {
      subjectSelButton.textContent = "Deseleccionar";
    } else subjectSelButton.textContent = "Seleccionar";
  ...
  createSubject(modal) {
    ...
    let creditsUDP = parseInt(modal.querySelector("#custom-credits-UDP").value);
    let creditsSCT = parseInt(modal.querySelector("#custom-credits-SCT").value);
    if (isNaN(creditsSCT)) creditsSCT = 0;
    let prer = [];

    let sectorName = "Custom";

    let subject = new SelectableRamo(
      name,
      sigla,
      creditsUDP,
      sectorName,
  ...
  createAdvancedSubject() {
    ...
    let creditsUDP = modal.querySelector("#custom-creditsa-UDP").value;
    let creditsSCT = modal.querySelector("#custom-creditsa-SCT").value;
    if (isNaN(parseInt(creditsUDP))) {
      creditsUDP = 1;
    } else creditsSCT = parseInt(creditsUDP);
    if (isNaN(parseInt(creditsSCT))) creditsSCT = 2;
    else creditsSCT = parseInt(creditsSCT);
  ...
  editSubject(subject) {
    ...
    let creditsUDP = modal.querySelector("#custom-creditsa-UDP").value;
    let creditsSCT = modal.querySelector("#custom-creditsa-SCT").value;
    if (creditsSCT.length === 0) creditsSCT = null;
    subject.updateCredits(creditsUDP, creditsSCT);
    subject.verifyPrer();
    if (!subject.beenEdited) this.subjectList.push(subject);
  ...
  deleteSubject(subject) {
    if (subject.selected) subject.selectRamo();
  ...
  restoreSubject(subject) {
    ...
    Object.values(this.semesterManager.malla.rawMalla).forEach(
      (subjectList) => {
        for (let rawSubject of subjectList) {
          if (rawSubject[1] === subject.sigla) {
            ...
            if (
              JSON.stringify(this.categories[rawSubject[4]]) !==
              JSON.stringify(
                this.semesterManager.malla.categories[rawSubject[4]]
              )
            ) {
            ...
            this.unDisplaySubject(subject);
            let i = this.subjectList.indexOf(subject);
            if (i > -1) this.subjectList.splice(i, 1);
            this.saveSubjects();
          }
        }
      }
  ...
  cleanSubjects() {
    ...
    listToClean.forEach((subject) => {
      if (subject.isCustom) this.deleteSubject(subject);
      else this.restoreSubject(subject);
    });
  ...
  saveSubjects() {
    ...
    this.subjectList.forEach((subject) => {
      ...
      if (subject.UDPtoSCT) cache[subject.sigla].push(0);
      else cache[subject.sigla].push(subject.getSCTCredits());
  ...
  loadSubjects() {
    ...
    if (Object.keys(prersNotFound).length !== 0) {
      ...
      Object.keys(prersNotFound).forEach((prer) => {
        let nestedList = list
          .append("li")
          .text(`Ramos que tenían a ${prer} como prerrequisito`)
          .append("ul");
    ...
    if (this.advanced) {
      ...
      if (cache) {
        ...
        for (let sigla in customSubjects) {
          ...
          if (this.semesterManager.malla.ALLSUBJECTS[sigla] === undefined) {
            let subject = new this.semesterManager.malla.subjectType(
              data[0],
              data[1],
              data[2],
              data[3],
  ...
  addPrerToModal(e, prerChooser = null) {
    let selector = null;
    if (prerChooser) selector = prerChooser;
    else selector = e.target;
  ...
  restoreCategory(categorySN = "Custom", a = null) {
    ...
    if (this.categories[categorySN] === undefined) this.createCategory(data);
    else this.editCategory(categorySN, data);
  }
  ...
  restoreCategories() {
    ...
    Object.keys(this.categories).forEach((category) => {
      if (categories[category] === undefined) this.deleteCategory(category);
    });
  ...
  displayCategory(categorySN) {
    ...
    if (this.needsWhiteText(color)) category.classList.add("text-white");
    ...
    category.addEventListener(
      "click",
      this.setUpCategoryModal.bind(this, true, categorySN)
    );
  ...
  updateCategory(categorySN) {
    ...
    else category.classList.remove("text-white");
    category.textContent = this.categories[categorySN][1];
  }
